...
// 有效扇区范围配置
#define VALID_SECTOR_START    10    // 有效起始扇区（扇区10）
#define VALID_SECTOR_END      62    // 有效结束扇区（扇区62）
#define PROTECTED_SECTOR      63    // 保护扇区（扇区63不可擦写）

// 管理记录扇区配置（扇区9，地址范围 0x00012000 - 0x00013FFF）
#define MANAGEMENT_SECTOR     9                      // 管理记录扇区号
...
struct FlashAdvHandle {
    ...
    // 初始化标志
    uint8_t initialized;
    uint8_t management_enabled;  // 管理记录是否启用
};
...
// 找到 FlashAdv_IsProtected 函数，替换为：
static uint8_t FlashAdv_IsProtected(FlashAdvHandle_t* handle, uint32_t address)
{
    if (!handle) return 1;
...
// 用户可用区域地址（基于扇区约束计算）
#define USER_REGION_START_ADDR  (VALID_SECTOR_START * 0x2000)   // 扇区10: 0x14000
#define USER_REGION_END_ADDR    ((VALID_SECTOR_END + 1) * 0x2000) // 扇区63起始: 0x7E000? 实际结束于0x7C000
// 根据HC32F46x的扇区布局：扇区0=0x00000, 扇区10=0x14000, 扇区62=0x7C000
...
void FlashAdv_GetUserRegion(uint32_t *start_addr, uint32_t *end_addr, uint32_t *size)
{
    ...
    if (end_addr) *end_addr = (VALID_SECTOR_END + 1) * 0x2000;  // 排除扇区62? 需要确认边界
