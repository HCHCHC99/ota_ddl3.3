...
static CanMsg_t     m_astcTxQueue[(32U)];
static volatile uint8_t m_u8TxHead = 0U;
static volatile uint8_t m_u8TxTail = 0U;

 
static CanIf_RxFilterEntry_t m_astcRxFilters[(16U)];
static uint8_t m_u8RxFilterCount = 0U;

 
static void (*m_pfnDefaultRxCallback)(const CanMsg_t *pMsg) = 0;
...
static _Bool CanIf_MatchFilter(const CanMsg_t *pMsg, const CanIf_RxFilterEntry_t *pFilter);
...
void CanIf_Init(void)
{
    ...
    nbDelay_Init(&m_stcBusOffTimer, (500U));

     



    CanIf_SetDefaultRxCallback(0);
...
_Bool CanIf_Send(const CanMsg_t *pMsg)
{
    ...
    {
        uint8_t u8Next = (m_u8TxHead + 1U) & ((32U) - 1U);
...
void CanIf_Poll(void)
{
    ...
    {
        CanMsg_t stcRxMsg;
        while (CanLLD_GetRxFrame(&stcRxMsg) == (0)) {
    ...
    if (CanLLD_IsBusOff()) {
        if (!m_stcBusOffTimer.isRunning) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[CANIF] Bus-Off detected, starting recovery timer (%ums)\r\n" "\033[0m" "\r\n", "MAIN",(500U));

            nbDelay_Start(&m_stcBusOffTimer);
        }
        if (nbDelay_IsComplete(&m_stcBusOffTimer)) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[CANIF] Bus-Off recovery: exiting local reset\r\n" "\033[0m" "\r\n", "MAIN");
...
uint8_t CanIf_GetTxQueueCount(void)
{
    uint8_t u8Count;
    __disable_irq();
    u8Count = (m_u8TxHead - m_u8TxTail) & ((32U) - 1U);
    __enable_irq();
    return u8Count;
}
...
static void CanIf_TxCompleteCallback(void)
{
    if (m_u8TxHead != m_u8TxTail) {
        ...
        m_u8TxTail = (m_u8TxTail + 1U) & ((32U) - 1U);
        (void)CanLLD_Send(&stcMsg);   
    }
}
...
static _Bool CanIf_MatchFilter(const CanMsg_t *pMsg, const CanIf_RxFilterEntry_t *pFilter)
{
    ...
    if (pFilter->u8Format == ((0x40000000UL)) && pMsg->u8IDE != 0U) {
        return 0;
    }
    if (pFilter->u8Format == ((0x40000000UL) | (0x20000000UL)) && pMsg->u8IDE == 0U) {
        return 0;
    }
...
static void CanIf_DispatchRx(const CanMsg_t *pMsg)
{
    ...
    if (!bMatched && m_pfnDefaultRxCallback != 0) {
