...
typedef enum {
    DMA_TRANS_MODE_SINGLE = 0,   
    DMA_TRANS_MODE_REPEAT = 1,   
} en_dma_trans_mode_t;
...
typedef struct {
    ...
    uint8_t         u8Channel;           
    en_dma_dir_t    enDir;               
    en_dma_trans_mode_t enTransMode;     
    
    uint32_t        u32SrcAddr;          
    uint32_t        u32DestAddr;         
    uint32_t        u32DataWidth;        
    uint16_t        u16BlockSize;        
    uint16_t        u16TransCount;       
...
typedef struct {
    ...
    stc_dma_config_t stcConfig;          
    
    uint16_t       *pu16Buffer;          
    uint16_t        u16BufferSize;       
    
    uint32_t        u32TransferCount;    
    uint8_t         u8DataUpdated;       
} stc_dma_instance_t;
...
static stc_adc_instance_t s_astcAdcInstances[(8U)];
static uint8_t s_u8AdcInstanceCount = 0;

 
static _Bool s_bAdcInitialized = 0;

 
static _Bool s_bHasDmaInstance = 0;

 
static int8_t s_a8AdcIdToDmaId[(8U)];
...
{
    switch (u16Pin) {
        case (0x0001U): return 0;
        case (0x0002U): return 1;
        case (0x0004U): return 2;
        case (0x0008U): return 3;
        case (0x0010U): return 4;
        case (0x0020U): return 5;
        case (0x0040U): return 6;
        case (0x0080U): return 7;
        case (0x0100U): return 8;
        case (0x0200U): return 9;
        case (0x0400U): return 10;
        case (0x0800U): return 11;
        case (0x1000U): return 12;
        case (0x2000U): return 13;
        case (0x4000U): return 14;
...
static void Adc_SetPinAnalogMode(uint8_t u8Port, uint8_t u8Pin)
{
    ...
    stcGpioInit.u16PinAttr = ((0x8000U));
	LL_PERIPH_WE((1UL << 2U));
...
static void Adc_InitConfig(void)
{
    ...
    FCG_Fcg3PeriphClockCmd((((0x00000001UL))), ENABLE);

     
    (void)ADC_StructInit(&stcAdcInit);
    stcAdcInit.u16ScanMode = (0x0U);   

     
    (void)ADC_Init((((CM_ADC_TypeDef *)(0x40040000UL))), &stcAdcInit);
    ...
    for (uint8_t i = 0; i < s_u8AdcInstanceCount; i++) {
        ...
        ADC_ChCmd((((CM_ADC_TypeDef *)(0x40040000UL))), (0U), pstcInst->u8Channel, ENABLE);
        ((void)0);
...
static void Timer0_Config(uint32_t u32IntervalUs)
{
    ...
    uint32_t u32TimerClk = CLK_GetBusClockFreq(((0x00000070UL))) / (1UL << (((8UL << (4U))) >> (4U)));
    ...
    stcTmr0Init.u32ClockDiv = ((8UL << (4U)));
    stcTmr0Init.u32Func = (0UL);
    stcTmr0Init.u16CompareValue = u16CompareValue;
    stcTmr0Init.u32ClockSrc = (0UL);
    
     
    (void)TMR0_Init((((CM_TMR0_TypeDef *)(0x40024000UL))), ((1UL)), &stcTmr0Init);
...
static void Adc_HardTriggerConfig(void)
{
    ...
    ADC_TriggerConfig((((CM_ADC_TypeDef *)(0x40040000UL))), (0U), (((0x0001U))));
    ADC_TriggerCmd((((CM_ADC_TypeDef *)(0x40040000UL))), (0U), ENABLE);
...
static void Adc_IrqConfig(void)
{
    ...
    if (u8HasInterruptMode) {
        ...
        __NVIC_ClearPendingIRQ(stcIrq.enIRQn);
        __NVIC_SetPriority(stcIrq.enIRQn, ((4U)));
        __NVIC_EnableIRQ(stcIrq.enIRQn);
        u8AdcIntEn |= ((0x01U));
        ((void)0);
    } else {
        ADC_IntCmd((((CM_ADC_TypeDef *)(0x40040000UL))), ((0x01U)), DISABLE);
        ((void)0);
    }
    
     
    if (u8AdcIntEn != 0U) {
        ADC_IntCmd((((CM_ADC_TypeDef *)(0x40040000UL))), u8AdcIntEn, ENABLE);
    }
}
...
static void Adc_ProcessInterruptChannels(void)
{
    ...
    for (uint8_t i = 0; i < s_u8AdcInstanceCount; i++) {
        ...
        if (pstcInst->enMode == ADC_MODE_INTERRUPT) {
            ...
            u16AdcValue = ADC_GetValue((((CM_ADC_TypeDef *)(0x40040000UL))), pstcInst->u8Channel);
...
static void ADC1_SeqA_IrqCallback(void)
{
    ...
    ADC_ClearStatus((((CM_ADC_TypeDef *)(0x40040000UL))), ((0x01U)));
...
static uint16_t Adc_CalcVoltage(uint16_t u16AdcValue)
{
    return (uint16_t)((((float32_t)(u16AdcValue) * (3.3F)) / ((float32_t)(1UL << 12U))) * 1000.F);
}
    ...
    if (s_bHasDmaInstance) {
        for (uint8_t i = 0; i < s_u8AdcInstanceCount; i++) {
            ...
            if (pstcInst->enMode == ADC_MODE_DMA) {
                ...
                stcDmaConfig.u32SrcAddr     = (uint32_t)((uint32_t)&(((CM_ADC_TypeDef *)(0x40040000UL)))->DR0 + (pstcInst->u8Channel * 2U));
                stcDmaConfig.u32DestAddr    = 0;   
                stcDmaConfig.u32DataWidth   = ((0x00000100UL));
                stcDmaConfig.u16BlockSize   = pstcInst->u16DmaBufferSize;
                stcDmaConfig.u16TransCount  = 0;   
                stcDmaConfig.u32SrcAddrInc  = (0x00000000UL);
                stcDmaConfig.u32DestAddrInc = ((0x00000004UL));
                stcDmaConfig.u8EnableInt    = 1;
                stcDmaConfig.u8IntPriority  = ((3U));
                stcDmaConfig.pfnCallback    = 0;   
...
void Adc_Stop(void)
{
    TMR0_Stop((((CM_TMR0_TypeDef *)(0x40024000UL))), ((1UL)));
    ((void)0);
}
...
void Adc_EnableInterrupt(void)
{
    ...
    ADC_IntCmd((((CM_ADC_TypeDef *)(0x40040000UL))), ((0x01U)), ENABLE);
