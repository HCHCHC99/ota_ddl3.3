#line 1 "..\\..\\App\\App_Comm.c"
#line 1 "..\\..\\App\\App_Comm.h"
...
#line 2 "..\\..\\App\\App_Comm.c"
#line 1 "..\\..\\Adp\\Comm_HAL.h"



#line 5 "..\\..\\Adp\\Comm_HAL.h"
#line 6 "..\\..\\Adp\\Comm_HAL.h"
#line 1 "..\\..\\Adp\\rs485.h"
...
#line 7 "..\\..\\Adp\\Comm_HAL.h"
#line 8 "..\\..\\Adp\\Comm_HAL.h"
...
_Bool Comm_HAL_Send(const uint8_t *data, uint16_t len);




 
_Bool Comm_HAL_RecvFrame(uint8_t *buf, uint16_t *len, uint16_t max_len);
...
#line 3 "..\\..\\App\\App_Comm.c"
#line 1 "..\\..\\App\\Protocol_ModbusRtu.h"



#line 5 "..\\..\\App\\Protocol_ModbusRtu.h"
#line 6 "..\\..\\App\\Protocol_ModbusRtu.h"
#line 7 "..\\..\\App\\Protocol_ModbusRtu.h"
...
#line 51 "..\\..\\App\\Protocol_ModbusRtu.h"
...
typedef _Bool (*ModbusRTU_ReadRegCallback_t)(uint16_t reg_addr, uint16_t *value);
typedef _Bool (*ModbusRTU_WriteRegCallback_t)(uint16_t reg_addr, uint16_t value);
typedef _Bool (*ModbusRTU_WriteMultiCallback_t)(uint16_t startReg, uint16_t regCount, const uint8_t *pData);
typedef _Bool (*ModbusRTU_ValidateRegCallback_t)(uint16_t reg_addr);

typedef struct {
    ModbusRTU_ReadRegCallback_t      on_read;
    ModbusRTU_WriteRegCallback_t     on_write;
    ModbusRTU_WriteMultiCallback_t   on_write_multi;  
...
#line 6 "..\\..\\App\\App_Comm.c"
#line 7 "..\\..\\App\\App_Comm.c"
#line 1 "..\\..\\App\\App_Realtime.h"



#line 5 "..\\..\\App\\App_Realtime.h"
...
int16_t Realtime_GetSpeed(void);




 
int16_t Realtime_GetAngle(void);




 
uint16_t Realtime_GetVoltage(void);




 
uint16_t Realtime_GetCurrent(void);




 
int16_t Realtime_GetDirection(void);

#line 8 "..\\..\\App\\App_Comm.c"
...
#line 10 "..\\..\\App\\App_Comm.c"
#line 11 "..\\..\\App\\App_Comm.c"



 
#line 21 "..\\..\\App\\App_Comm.c"
...
static _Bool m_bCommControlEnabled;
static _Bool m_bResetPending;
...
static const Param_Config_t m_stcParamConfig = {
    ...
    .magicHead      = 0x55AA55AA,
    .magicTail      = 0xAA44AA44,
    .checksumOffset = (( size_t)__INTADDR__(&(((AppParamRecord_t *)0)->checksum))),
    .seqOffset      = (( size_t)__INTADDR__(&(((AppParamRecord_t *)0)->sequence_id))),
    .eraseCntOffset = (( size_t)__INTADDR__(&(((AppParamRecord_t *)0)->erase_count))),
};
...
static _Bool App_Comm_OnReadReg(uint16_t reg_addr, uint16_t *value);
static _Bool App_Comm_OnWriteReg(uint16_t reg_addr, uint16_t value);
static _Bool App_Comm_OnValidateReg(uint16_t reg_addr);
...
static void App_Comm_SetParamDefaults(void)
{
    ...
    g_AppParam.head_magic = 0x55AA55AA;
    g_AppParam.tail_magic = 0xAA44AA44;

    g_AppParam.node_id               = (uint16_t)(1U);
    g_AppParam.target_speed          = (int16_t)(0);
    g_AppParam.target_angle          = (int16_t)(0);
    g_AppParam.voltage_upper_limit   = (uint16_t)(270U);
    g_AppParam.voltage_lower_limit   = (uint16_t)(210U);
    g_AppParam.current_upper_limit   = (uint16_t)(5000);
    g_AppParam.current_detect_ms     = (uint16_t)(20);
    g_AppParam.close_limit_angle     = (int16_t)(-20);
    g_AppParam.open_limit_angle      = (int16_t)(880);
    g_AppParam.baud_rate             = (9600UL);

    g_AppParam.voltage_upper_hysteresis   = (uint16_t)(20U);
    g_AppParam.voltage_lower_hysteresis   = (uint16_t)(20U);
    g_AppParam.overvoltage_trigger_count  = (uint8_t)(1U);
    g_AppParam.undervoltage_trigger_count = (uint8_t)(1U);
    g_AppParam.current_hysteresis_ma      = (uint16_t)(0U);
    g_AppParam.current_release_ms         = (uint16_t)(200U);
    g_AppParam.overcurrent_trigger_count  = (uint8_t)(1U);
...
static _Bool App_Comm_OnReadReg(uint16_t reg_addr, uint16_t *value)
{
    ...
    default:
        return (Param_ReadByReg(reg_addr, value) == 0);
    }
    return 1;
}
...
static _Bool App_Comm_OnWriteMulti(uint16_t startReg, uint16_t regCount, const uint8_t *pData)
{
    uint16_t i;
    uint16_t regAddr;
    uint16_t regValue;
    ...
    {
        regAddr = startReg + i;
        if (regAddr == (0x2720U) || regAddr == (0x2740U))
        {
            ((void)0);
            return 0;
        }
    ...
    for (i = 0U; i < regCount; i++)
    {
        ...
        if (Param_WriteByReg(regAddr, regValue) != 0)
        {
            ((void)0);
            return 0;
        }
    ...
    if (Param_Save(&m_stcParamConfig) != 0)
    {
        ((void)0);
        return 0;
    }
...
static void App_Comm_HandleCtrlCmd(uint16_t cmd)
{
    ...
    if (cmd & (0x0008U))
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] RESET command: start reset delay %dms" "\033[0m" "\r\n", "MAIN",(int)(200U));
        m_bResetPending = 1;
    ...
    if (cmd & (0x0002U))
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] STOP: RS485 control DISABLED, motor STOP" "\033[0m" "\r\n", "MAIN");
        m_bCommControlEnabled = 0;
    ...
    if (cmd & (0x0004U))
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] ESTOP: emergency stop, control still ENABLED" "\033[0m" "\r\n", "MAIN");
    ...
    if (cmd & (0x0001U))
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] START: RS485 control ENABLED" "\033[0m" "\r\n", "MAIN");
        m_bCommControlEnabled = 1;
        return;
    }
    ...
    if (!m_bCommControlEnabled)
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] ERROR: FWD/REV rejected - control not enabled (send START first)" "\033[0m" "\r\n", "MAIN");
        return;
    }
    ...
    if (cmd & (0x0010U))
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] FWD: speed=%.1f%%" "\033[0m" "\r\n", "MAIN",target_speed_percent);
    ...
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] REV: speed=%.1f%%" "\033[0m" "\r\n", "MAIN",target_speed_percent);
...
void App_Comm_Init(const App_Comm_Config_t *cfg)
{
    ...
    nbDelay_Init(&m_stcResetDelay, (200U));
    m_bResetPending        = 0;
    m_bCommControlEnabled  = 0;
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "App_Comm initialized: node_id=%d, baud=%lu" "\033[0m" "\r\n", "MAIN",g_AppParam . node_id, g_AppParam . baud_rate);
...
void App_Comm_Poll(void)
{
    ...
    {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP_COMM] Reset delay done, system reset now!" "\033[0m" "\r\n", "MAIN");
        m_bResetPending = 0;
        __NVIC_SystemReset();
    }
}
