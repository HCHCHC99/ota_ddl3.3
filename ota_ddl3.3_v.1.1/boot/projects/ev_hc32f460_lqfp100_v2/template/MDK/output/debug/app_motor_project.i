#line 1 "..\\..\\App\\App_Motor_Project.c"
...
#line 2 "..\\..\\App\\App_Motor_Project.c"
#line 3 "..\\..\\App\\App_Motor_Project.c"
#line 4 "..\\..\\App\\App_Motor_Project.c"
#line 5 "..\\..\\App\\App_Motor_Project.c"
#line 6 "..\\..\\App\\App_Motor_Project.c"
#line 7 "..\\..\\App\\App_Motor_Project.c"
#line 8 "..\\..\\App\\App_Motor_Project.c"
...
#line 9 "..\\..\\App\\App_Motor_Project.c"
...
RTurn_Device_t* g_rturn_dev = 0;


static PWM_Device_t g_pwm_dev;
static MotorDevice_t g_motor_dev;

MotorDevice_t* volatile g_pMotorDevWatch = 0;
static Power_Device_t* g_pwr_pos_dev = 0;
static Power_Device_t* g_pwr_neg_dev = 0;
static IO_Device_t* g_io_fwd_dev = 0;
static IO_Device_t* g_io_rev_dev = 0;
static Hall_Device_t* g_hall_up_dev = 0;
static Hall_Device_t* g_hall_down_dev = 0;
static Voltage_Device_t* g_voltage_bus_dev = 0;
static Sensor_Device_t* g_sensor_current_dev = 0;
...
static void RegisterAllDevices(void) {
    ...
    DeviceManager_Register(1, "PowerPositive", DEVICE_TYPE_POWER, 
    ...
    DeviceManager_Register(0, "MotorArbitrator", DEVICE_TYPE_MOTOR, 
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Before Register: motor_hall_dev=0x%p\r\n" "\033[0m" "\r\n", "MAIN",motor_hall_dev);
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Before Register: motor_hall_dev->config.hall_a_pin=0x%04X\r\n" "\033[0m" "\r\n", "MAIN",motor_hall_dev->config . hall_a_pin);

    DeviceManager_Register(10, "MotorHall", DEVICE_TYPE_HALL, 
                        motor_hall_dev, g_motor_hall_ops);

    
    DeviceNode_t* check_node = DeviceManager_Get(10);
    if (check_node && check_node->private_data) {
        MotorHall_Device_t* check_dev = (MotorHall_Device_t*)check_node->private_data;
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "After Register: check_dev=0x%p\r\n" "\033[0m" "\r\n", "MAIN",check_dev);
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "After Register: check_dev->config.hall_a_pin=0x%04X\r\n" "\033[0m" "\r\n", "MAIN",check_dev->config . hall_a_pin);
    }                    
    ...
    ADC_Config_t adcCurrentCfg = {
        ...
        .u8Port = (0x00U),
        .u16Pin = (0x0020U),
        .enAcqMode = ADC_ACQ_MODE_INTERRUPT,
        .u16DmaBufferSize = 0,
        .u8DmaChannel = 0
    };

    ADC_Device_t* adc_current_dev = ADC_Device_Create(&adcCurrentCfg);
    DeviceManager_Register(11, "ADC_Current", DEVICE_TYPE_ADC, 
    ...
    DeviceManager_Register(12, "ADC_Voltage", DEVICE_TYPE_ADC, 
    ...
    
    {
        ...
        if (u32OvervoltageThresholdMv == 0) {
            u32OvervoltageThresholdMv = (270U) * 100UL;
            ((void)0);
        }
        if (u32UndervoltageThresholdMv == 0) {
            u32UndervoltageThresholdMv = (210U) * 100UL;
            ((void)0);
        }
        if (u32OvervoltageHysteresisMv == 0) {
            u32OvervoltageHysteresisMv = (20U) * 100UL;
        }
        if (u32UndervoltageHysteresisMv == 0) {
            u32UndervoltageHysteresisMv = (20U) * 100UL;
        }
        if (u8OvervoltageTriggerCount == 0) {
            u8OvervoltageTriggerCount = (1U);
        }
        if (u8UndervoltageTriggerCount == 0) {
            u8UndervoltageTriggerCount = (1U);
        }
        
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP] Voltage config from Flash: upper=%lu mV (0.1V=%d), lower=%lu mV (0.1V=%d), " "upper_hys=%lu mV, lower_hys=%lu mV, ov_cnt=%d, uv_cnt=%d" "\033[0m" "\r\n", "MAIN",u32OvervoltageThresholdMv, g_AppParam . voltage_upper_limit, u32UndervoltageThresholdMv, g_AppParam . voltage_lower_limit, u32OvervoltageHysteresisMv, u32UndervoltageHysteresisMv, u8OvervoltageTriggerCount, u8UndervoltageTriggerCount);
        ...
        DeviceManager_Register(13, "VoltageBus", DEVICE_TYPE_SENSOR,
    ...
    
    {
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP] Sensor config from Flash: threshold=%ldmA, trigger_window=%ldms, release_window=%ldms, hysteresis=%ldmA, trigger_cnt=%d" "\033[0m" "\r\n", "MAIN",(long)s32ThresholdMa, (long)u32TriggerMs, (long)u32ReleaseMs, (long)s32HysteresisMa, (int)u8TriggerCount);
        ...
        DeviceManager_Register(14, "SensorCurrent", DEVICE_TYPE_SENSOR,
    ...
    RTurn_Config_t rturnCfg = {
        .u8MotorHallDevId = 10,
        .u8MotorArbiterDevId = 0,
        .u8SensorDevId = 14,
        .fReductionRatio = (1183.0f),
        .fMaxAngle = (88.0f),
        .fMinAngle = (-2.0f),
        .u8ReverseOutput = (0),
        .u8DeviceId = 15,
        .u16UpdateIntervalMs = (1)
    };

    g_rturn_dev = RTurn_Device_Create(&rturnCfg);
    DeviceManager_Register(15, "RTurn", DEVICE_TYPE_SENSOR, 
...
static void SetDeviceUpdateIntervals(void) {
    DeviceManager_SetUpdateInterval(1, 1);
    DeviceManager_SetUpdateInterval(2, 1);
    DeviceManager_SetUpdateInterval(0, 1);
    DeviceManager_SetUpdateInterval(10, 1);
    DeviceManager_SetUpdateInterval(11, 1);
    DeviceManager_SetUpdateInterval(12, 1);
    DeviceManager_SetUpdateInterval(13, 10);
    DeviceManager_SetUpdateInterval(14, 1);
    DeviceManager_SetUpdateInterval(15, 1);
}
...
void Sim_PublishEvents(void) {
    ...
    if (g_sim.sim_pwr_pos != last_pwr_pos) {
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] POWER_POS changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_pwr_pos, g_sim . sim_pwr_pos);
    ...
    if (g_sim.sim_pwr_neg != last_pwr_neg) {
        ...
        EventBus_Publish(TOPIC_POWER, &pwrEvent);
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] POWER_NEG changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_pwr_neg, g_sim . sim_pwr_neg);
    ...
    if (g_sim.sim_hall_up != last_hall_up) {
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] HALL_UP changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_hall_up, g_sim . sim_hall_up);
    ...
    if (g_sim.sim_hall_down != last_hall_down) {
        ...
        EventBus_Publish(TOPIC_LIMIT_HARD, &limitEvent);
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] HALL_DOWN changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_hall_down, g_sim . sim_hall_down);
    ...
    if (g_sim.sim_io_fwd != last_io_fwd) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] IO_FWD changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_io_fwd, g_sim . sim_io_fwd);
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] Publishing IO_FWD event: dir=%d, type=%d, speed=%ld.%ld%%\r\n" "\033[0m" "\r\n", "MAIN",ioEvent . dir, ioEvent . type, (long)(speed_int / 10), (long)(speed_int % 10));
    ...
    if (g_sim.sim_io_rev != last_io_rev) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] IO_REV changed: %d -> %d\r\n" "\033[0m" "\r\n", "MAIN",last_io_rev, g_sim . sim_io_rev);
        ...
        int32_t speed_int = (int32_t)(ioEvent.speed * 10);
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SIM] Publishing IO_REV event: dir=%d, type=%d, speed=%ld.%ld%%\r\n" "\033[0m" "\r\n", "MAIN",ioEvent . dir, ioEvent . type, (long)(speed_int / 10), (long)(speed_int % 10));
...
void ESystem_Init(void) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP] Initializing motor arbiter first...\r\n" "\033[0m" "\r\n", "MAIN");
    Device_Init(0);
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[APP] Initializing remaining devices...\r\n" "\033[0m" "\r\n", "MAIN");
