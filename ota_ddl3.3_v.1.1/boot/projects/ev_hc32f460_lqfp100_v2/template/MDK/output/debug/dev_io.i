...
static uint8_t IO_ReadPinState(IO_Device_t* dev) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO GPIO Read: ID=%d, Port=%d, Pin=0x%04X, PinState=%d, PortValue=0x%04X, CalculatedState=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, dev->port, dev->pin, pin_state, port_value, state);

    
    
    uint8_t calculated_from_port = ((port_value & dev->pin) != 0) ? 1 : 0;
    if (state != calculated_from_port) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "WARNING: Pin state mismatch! Direct=%d, FromPort=%d\r\n" "\033[0m" "\r\n", "MAIN",state, calculated_from_port);
    }
...
DeviceResult_t IO_Device_Init(void* handle) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO init: ID=%d, Port=%d, Pin=%d, raw=%d, active_level=%d, init=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, dev->port, dev->pin, raw_state, dev->active_level, init_state);
...
DeviceResult_t IO_Device_Update(void* handle) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO Update Start: ID=%d, Now=%u, LastSample=%u, Interval=%d, Diff=%u\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, (unsigned int)now, (unsigned int)dev->last_sample_time, dev->sample_interval, (unsigned int)(now - dev->last_sample_time));


    
    if (now - dev->last_sample_time < dev->sample_interval) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO Update Skip: ID=%d, Sample interval not reached\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id);
        return RESULT_OK;
    }
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO State Calc: ID=%d, Raw=%d, ActiveLevel=%d, ActiveState=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, raw_state, dev->active_level, active_state);

    
    uint8_t filtered_state = IO_SoftwareFilter(dev, active_state);
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO State Check: ID=%d, Filtered=%d, LastStable=%d, LastChange=%u, Debounce=%d, Diff=%u\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, filtered_state, dev->last_stable_state, (unsigned int)dev->last_change_time, dev->debounce_ms, (unsigned int)(now - dev->last_change_time));
    ...
    if (filtered_state != dev->last_stable_state) {
        if (now - dev->last_change_time >= dev->debounce_ms) {
            ...
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO State Changed: ID=%d, Old=%d, New=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, old_state, filtered_state);

            IO_PublishEvent(dev, filtered_state, old_state);
        } else {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO Debounce Waiting: ID=%d, Need %u more ms\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id, (unsigned int)(dev->debounce_ms - (now - dev->last_change_time)));

        }
    } else {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "IO State Stable: ID=%d, No change\r\n" "\033[0m" "\r\n", "MAIN",dev->io_id);
    }
