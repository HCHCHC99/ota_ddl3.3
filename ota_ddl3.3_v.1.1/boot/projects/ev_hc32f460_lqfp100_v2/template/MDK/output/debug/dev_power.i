...
#line 2 "..\\..\\Dev\\dev_power.c"
#line 3 "..\\..\\Dev\\dev_power.c"
#line 4 "..\\..\\Dev\\dev_power.c"
#line 5 "..\\..\\Dev\\dev_power.c"
#line 6 "..\\..\\Dev\\dev_power.c"
#line 7 "..\\..\\Dev\\dev_power.c"


    static uint8_t s_u8SimPowerState = 1;  
    static uint8_t s_u8LastSimPowerState = 1;



static uint8_t Power_ReadPinState(Power_Device_t* dev) {
    en_pin_state_t pin_state = GPIO_ReadInputPins(dev->port, dev->pin);
    uint8_t state = (pin_state == PIN_SET) ? 1 : 0;
    return state;
}


static uint8_t Power_SoftwareFilter(Power_Device_t* dev, uint8_t new_sample) {
...
static void Power_PublishEvent(Power_Device_t* dev, uint8_t new_state) {
    MotorPowerEvent_t powerEvent = {
        .power_id = dev->power_id,
        .is_on = (new_state == 1)
    };
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] Publishing: ID=%d, is_on=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id, new_state);  
    
    EventBus_Publish(TOPIC_POWER, &powerEvent);
}



DeviceResult_t Power_Device_Init(void* handle) {
    Power_Device_t* dev = (Power_Device_t*)handle;
    if (!dev) return RESULT_PARAM_ERR;
    
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] === Init called: ID=%d ===\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id);  
    

    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] SIM mode: ID=%d, SimState=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id, s_u8SimPowerState);
    
    dev->initialized = 1;
    dev->current_state = s_u8SimPowerState;
    dev->last_stable_state = s_u8SimPowerState;
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] Init success (SIM): ID=%d, state=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id, dev->current_state);
    
    
    Power_PublishEvent(dev, dev->current_state);
    
    return RESULT_OK;
#line 110 "..\\..\\Dev\\dev_power.c"
}
...
DeviceResult_t Power_Device_Deinit(void* handle) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] Deinit: ID=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id);
    dev->initialized = 0;
    return RESULT_OK;
}
...
DeviceResult_t Power_Device_Update(void* handle) {
    ...
    if (s_u8SimPowerState != s_u8LastSimPowerState) {
        s_u8LastSimPowerState = s_u8SimPowerState;
        dev->current_state = s_u8SimPowerState;
        dev->last_stable_state = s_u8SimPowerState;
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] SIM state changed: ID=%d, NewState=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id, dev->current_state);
        Power_PublishEvent(dev, dev->current_state);
    }
    return RESULT_OK;
#line 205 "..\\..\\Dev\\dev_power.c"
}
...
Power_Device_t* Power_Device_Create(const Power_Config_t* config) {
    ...
    dev->power_id = config->power_id;
    dev->debounce_ms = config->debounce_ms;
    dev->window_size = config->window_size;
    dev->sample_interval = config->sample_interval;
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] Created: ID=%d, Port=%d, Pin=%d\r\n" "\033[0m" "\r\n", "MAIN",dev->power_id, dev->port, dev->pin);
...
void Power_SetSimulationState(uint8_t u8State) {
    s_u8SimPowerState = (u8State != 0) ? 1 : 0;
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[POWER] SIM state set to %d\r\n" "\033[0m" "\r\n", "MAIN",s_u8SimPowerState);
}


const DeviceOps_t g_power_ops = {
    .init = Power_Device_Init,
    .deinit = Power_Device_Deinit,
    .read = Power_Device_Read,
    .write = Power_Device_Write,
    .control = Power_Device_Control,
    .update = Power_Device_Update
};
