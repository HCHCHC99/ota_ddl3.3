...
#line 2 "..\\..\\Dev\\dev_sensor.c"
#line 3 "..\\..\\Dev\\dev_sensor.c"
#line 4 "..\\..\\Dev\\dev_sensor.c"
#line 5 "..\\..\\Dev\\dev_sensor.c"
#line 6 "..\\..\\Dev\\dev_sensor.c"


#line 38 "..\\..\\Dev\\dev_sensor.c"
    ...
    static uint8_t Sensor_RealDbg_IsTime(void) {
        if (!s_u8RealDbgTimerInit) {
            nbDelay_Init(&s_stcRealDbgTimer, (3000));
    ...
    volatile uint16_t g_u16DbgSensorBuffer[(200)] = {0};
    ...
    static void Sensor_Debug_AddToWindow(uint16_t u16VoltageMv) {
        ...
        if (g_u16DbgSensorBufIndex >= (200)) {
            g_u16DbgSensorBufIndex = 0;
            g_u8DbgSensorBufOverflow = 1;
        }
        if (g_u16DbgSensorBufCount < (200)) {
...
static void Sensor_CalibrateZeroInternal(Sensor_Device_t* pstcDev, uint16_t u16AdcVoltageMv) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "DiffAmp Calib: Meas=%ld mV, Offset=%ld mV\r\n" "\033[0m" "\r\n", "MAIN",(long)s32ZeroMeas, (long)s32ZeroOffset);
#line 119 "..\\..\\Dev\\dev_sensor.c"
...
#line 189 "..\\..\\Dev\\dev_sensor.c"
    ...
    if (Sensor_RealDbg_IsTime()) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR_REAL] " "ADC_Read: id=%d res=%d raw=%d mV=%d\r\n" "\033[0m" "\r\n", "MAIN",pstcDev->stcConfig . u8AdcDevId, res, stcAdcResp . u16RawValue, stcAdcResp . u16VoltageMv);
...
static void Sensor_CheckOvercurrent(Sensor_Device_t* pstcDev) {
    ...
    if (pstcCfg->u8OvercurrentMode == 0) {
...
DeviceResult_t Sensor_Device_Init(void* handle) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Init: ADC Dev ID=%d\r\n" "\033[0m" "\r\n", "MAIN",pstcDev->stcConfig . u8AdcDevId);
    

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Mode: Differential Amplifier ENABLED (Zero=0mV, Sens=100mV/A)\r\n" "\033[0m" "\r\n", "MAIN");
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Calibration Range: %d ~ %d mV\r\n" "\033[0m" "\r\n", "MAIN",(10), (150));
#line 394 "..\\..\\Dev\\dev_sensor.c"
    
#line 401 "..\\..\\Dev\\dev_sensor.c"
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Mode: Voltage Divider DISABLED\r\n" "\033[0m" "\r\n", "MAIN");
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Init success: ADC mV=%d, Current=%ld mA (waiting for calibration)\r\n" "\033[0m" "\r\n", "MAIN",pstcDev->u16AdcVoltageMv, (long)pstcDev->s32CurrentMa);
...
DeviceResult_t Sensor_Device_Update(void* handle) {
    ...
    if (!pstcDev->u8Calibrated) {
        ...
        if (u16Voltage >= (10) && 
            u16Voltage <= (150)) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Detected zero current state: %d mV (elapsed=%lu ms), calibrating...\r\n" "\033[0m" "\r\n", "MAIN",u16Voltage, (unsigned long)u32Elapsed);

            Sensor_CalibrateZeroInternal(pstcDev, u16Voltage);
            pstcDev->u8Calibrated = 1;
        } else if (u32Elapsed > 5000) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Calibration timeout! Using theoretical zero\r\n" "\033[0m" "\r\n", "MAIN");
    ...
    uint32_t u32Now = tickTimer_GetCount();
    if (u32Now - s_u32LastSlowPrintTime >= 4000) {
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Current=%s%ld.%02ldA (ADC Raw=%d, ADC mV=%d)%s\r\n" "\033[0m" "\r\n", "MAIN",(pstcDev->s16CurrentAx100 < 0) ? "-" : "", (long)(s16AbsAx100 / 100), (long)(s16AbsAx100 % 100), pstcDev->u16AdcRawValue, pstcDev->u16AdcVoltageMv, pstcDev->stcAlarmState . u8OvercurrentAlarm ? " [OVERCURRENT]" : "");
#line 491 "..\\..\\Dev\\dev_sensor.c"
    }
...
void Sensor_Device_CalibrateZero(Sensor_Device_t* pstcDev) {
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "[SENSOR] " "Manual calibration done: ZeroOffset=%ld mV\r\n" "\033[0m" "\r\n", "MAIN",(long)pstcDev->stcCalibration . s32ZeroOffsetMv);
