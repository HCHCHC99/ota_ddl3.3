...
static stc_dma_instance_t s_astcDmaInstances[(8U)];
...
static int8_t s_a8DmaIrqMap[2][(4U)];
...
static CM_DMA_TypeDef* Dma_GetUnitPtr(uint8_t u8DmaUnit)
{
    return (u8DmaUnit == 1) ? ((CM_DMA_TypeDef *)(0x40053000UL)) : ((CM_DMA_TypeDef *)(0x40053400UL));
}
...
static uint32_t Dma_GetPeriphClk(uint8_t u8DmaUnit)
{
    return (u8DmaUnit == 1) ? ((0x00004000UL)) : ((0x00008000UL));
}
...
static void Dma_IrqConfig(stc_dma_instance_t *pstcInst)
{
    ...
    DMA_ClearTransCompleteStatus(pDma, (((0x00010000UL)) << u8Ch));

    __NVIC_ClearPendingIRQ(enIRQn);
    __NVIC_SetPriority(enIRQn, pstcInst->stcConfig.u8IntPriority);
    __NVIC_EnableIRQ(enIRQn);
...
static void Dma_IrqHandler(uint8_t u8DmaUnit, uint8_t u8Ch)
{
    ...
    if (s8Id >= 0 && s8Id < (int8_t)s_u8DmaInstanceCount) {
        ...
        if (pstcInst->stcConfig.pfnCallback != 0) {
...
void Dma_Init(void)
{
    ...
    for (uint8_t i = 0; i < s_u8DmaInstanceCount; i++) {
        ...
        stcDmaInit.u32IntEn       = pstcInst->stcConfig.u8EnableInt ? ((0x00001000UL)) : (0x00000000UL);
        ...
        if (pstcInst->stcConfig.enTransMode == DMA_TRANS_MODE_REPEAT) {
            ...
            stcDmaRptInit.u32Mode      = ((0x00000010UL) | (0x00000020UL));
...
uint16_t Dma_GetLatestValue(uint8_t u8Ch)
{
    if (u8Ch >= s_u8DmaInstanceCount) return 0;
    stc_dma_instance_t *pstcInst = &s_astcDmaInstances[u8Ch];
    if (pstcInst->pu16Buffer != 0 && pstcInst->u16BufferSize > 0) {
...
uint8_t Dma_IsChannelBusy(uint8_t u8Ch)
{
    ...
    switch (pstcInst->stcConfig.u8Channel) {
        case 0: u32Status = ((0x00010000UL)); break;
        case 1: u32Status = ((0x00020000UL)); break;
        case 2: u32Status = ((0x00040000UL)); break;
        case 3: u32Status = ((0x00080000UL)); break;
        default: return 0;
    }
...
uint8_t Dma_IsTransferComplete(uint8_t u8Ch)
{
    ...
    uint32_t u32Flag = 0;
    switch (pstcInst->stcConfig.u8Channel) {
        case 0: u32Flag = ((0x00000001UL)); break;
        case 1: u32Flag = ((0x00000002UL)); break;
        case 2: u32Flag = ((0x00000004UL)); break;
        case 3: u32Flag = ((0x00000008UL)); break;
        default: return 0;
    }
...
uint8_t Dma_IsBlockComplete(uint8_t u8Ch)
{
    ...
    uint32_t u32Flag = 0;
    switch (pstcInst->stcConfig.u8Channel) {
        case 0: u32Flag = ((0x00010000UL)); break;
        case 1: u32Flag = ((0x00020000UL)); break;
        case 2: u32Flag = ((0x00040000UL)); break;
        case 3: u32Flag = ((0x00080000UL)); break;
        default: return 0;
    }
...
uint8_t Dma_IsError(uint8_t u8Ch)
{
    ...
    switch (pstcInst->stcConfig.u8Channel) {
        case 0:
            u32ReqErr = ((0x00010000UL));
            u32TransErr = ((0x00000001UL));
            break;
        case 1:
            u32ReqErr = ((0x00020000UL));
            u32TransErr = ((0x00000002UL));
            break;
        case 2:
            u32ReqErr = ((0x00040000UL));
            u32TransErr = ((0x00000004UL));
            break;
        case 3:
            u32ReqErr = ((0x00080000UL));
            u32TransErr = ((0x00000008UL));
            break;
        default: return 0;
    }
...
void Dma_ClearFlag(uint8_t u8Ch, uint32_t u32Flag)
{
    ...
    if ((u32Flag == ((0x00010000UL))) || (u32Flag == ((0x00020000UL))) ||
        (u32Flag == ((0x00040000UL))) || (u32Flag == ((0x00080000UL))) ||
        (u32Flag == ((0x00000001UL))) || (u32Flag == ((0x00000002UL))) ||
        (u32Flag == ((0x00000004UL))) || (u32Flag == ((0x00000008UL)))) {
...
void Dma_PrintDebugInfo(void)
{
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "=== DMA Driver Debug Info ===\r\n" "\033[0m" "\r\n", "MAIN");
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Initialized: %s\r\n" "\033[0m" "\r\n", "MAIN",s_bDmaInitialized ? "Yes" : "No");
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Instance count: %d\r\n" "\033[0m" "\r\n", "MAIN",s_u8DmaInstanceCount);

    for (uint8_t i = 0; i < s_u8DmaInstanceCount; i++) {
        stc_dma_instance_t *pstcInst = &s_astcDmaInstances[i];
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  ID%d: DMA%d CH%d, Dir=%s, Block=%d, Buffer=0x%08X, Count=%lu\r\n" "\033[0m" "\r\n", "MAIN",pstcInst->u8Id, pstcInst->stcConfig . u8DmaUnit, pstcInst->stcConfig . u8Channel, (pstcInst->stcConfig . enDir == DMA_DIR_PERIPH_TO_MEM) ? "Periph->Mem" : (pstcInst->stcConfig . enDir == DMA_DIR_MEM_TO_PERIPH) ? "Mem->Periph" : "Mem->Mem", pstcInst->stcConfig . u16BlockSize, (uint32_t)pstcInst->pu16Buffer, pstcInst->u32TransferCount);
#line 680 "..\\..\\Adp\\Dma.c"
    }
}
