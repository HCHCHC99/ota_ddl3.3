...
struct FlashAdvHandle {
    ...
    FlashAdvHistoryRecord_t history[16];
    uint32_t total_operations;
...
FlashAdvStatus_t FlashAdv_SaveLifetimeInfo(FlashAdvHandle_t* handle)
{
    ...
    save_info.magic = 0x5A5A5A5A;
    save_info.version = 0x00000001;
    save_info.checksum = FlashAdv_CalculateChecksum(&save_info);
    
    
    FlashAdvStatus_t status = handle->ops.erase_sector((0x00012000));
...
FlashAdvStatus_t FlashAdv_LoadLifetimeInfo(FlashAdvHandle_t* handle)
{
    ...
    for (uint32_t i = 0; i < sizeof(FlashAdvLifetimeInfo_t) / 4; i++) {
        read_ptr[i] = handle->ops.read_word((0x00012000) + i * 4);
    }
    
    
    if (loaded_info.magic == 0x5A5A5A5A && 
        loaded_info.version == 0x00000001) {
                                      ...
                                      FlashAdvStatus_t status)
{
    ...
    uint32_t idx = handle->total_operations % 16;
...
static void FlashAdv_UpdateLifetime(FlashAdvHandle_t* handle, uint32_t sector_idx)
{
    ...
    for (uint32_t i = 0; i < handle->lifetime.sector_count && i < 64; i++) {
...
FlashAdvHandle_t* FlashAdv_Create(const FlashAdvConfig_t* config, const FlashAdvOps_t* ops)
{
    ...
    g_flash_adv_handle.lifetime.valid_start_sector = 13;
    g_flash_adv_handle.lifetime.valid_end_sector = 61;
    
    if (g_flash_adv_handle.lifetime.sector_count > 64) {
        g_flash_adv_handle.lifetime.sector_count = 64;
    }
                                      ...
                                      uint32_t* num_returned)
{
    ...
    uint32_t start_idx = (handle->total_operations > handle->history_count) ?
                         (handle->total_operations % 16) : 0;
    
    for (uint32_t i = 0; i < copy_count; i++) {
        uint32_t idx = (start_idx + i) % 16;
...
FlashAdvStatus_t FlashAdv_GetLastOperation(FlashAdvHandle_t* handle, FlashAdvHistoryRecord_t* record)
{
    ...
    uint32_t last_idx = (handle->total_operations - 1) % 16;
...
uint32_t flash_advanced_test(void)
{
    ...
    FlashAdvHandle_t* flash = FlashAdv_Create(&config, 0);
    if (!flash) {
    ...
    uint32_t verify_data[8];
    
    uint32_t sector_start = FlashAdv_GetSectorStart(flash, 0x00050000);
    ...
    status = FlashAdv_BulkWriteSimple(flash, 0x00050000, test_data, 8);
    if (status != FLASH_ADV_OK) {
        g_test_result = 2;
        return g_test_result;
    }
    ...
    for (uint32_t i = 0; i < 8; i++) {
        verify_data[i] = FlashAdv_ReadWord(flash, 0x00050000 + (i * 4));
...
void FlashAdv_GetUserRegion(uint32_t *start_addr, uint32_t *end_addr, uint32_t *size)
{
    if (start_addr) *start_addr = 13 * 0x2000;
    if (end_addr) *end_addr = (61 + 1) * 0x2000;  
    if (size) *size = (61 - 13 + 1) * 0x2000;
}
