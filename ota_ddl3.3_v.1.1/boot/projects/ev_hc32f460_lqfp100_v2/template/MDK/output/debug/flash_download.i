...
typedef struct {
    ...
    uint32_t current_max_erase;         
    uint32_t current_min_erase;         
    uint32_t total_erase_count;         
    uint32_t sector_size;               
    uint32_t sector_erase_counts[64];  
    uint16_t sector_count;              
    uint8_t health_percent;             
    uint32_t valid_start_sector;        
    uint32_t valid_end_sector;          
    uint32_t checksum;                  
...
{
    return ((address) >= (13 * 0x2000) && (address) < ((61 + 1) * 0x2000));
}
...
{
    if (start_sector) *start_sector = 0;
    if (end_sector) *end_sector = 12;
}
...
static void fw_set_state(FlashDownloadState_t new_state)
{
    if (g_fw_ctx.state != new_state) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "State: %d -> %d" "\033[0m" "\r\n", "FW",g_fw_ctx . state, new_state);
...
static void fw_set_error(FlashDownloadResult_t error)
{
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Error set: %d" "\033[0m" "\r\n", "FW",error);
}
...
static _Bool fw_is_address_valid(uint32_t address)
{
    ...
    uint32_t mapped_addr = (((address) >= 0x08004000 && (address) < 0x08010000) ? ((address) - 0x08004000 + 0x0001A000) : (address));
    return (mapped_addr >= 0x0001A000 && 
            mapped_addr < (0x0001A000 + 0x0000C000));
...
static FlashDownloadResult_t fw_erase_range(uint32_t start_addr, uint32_t end_addr)
{
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Erasing range: 0x%08X - 0x%08X" "\033[0m" "\r\n", "FW",start_sector, end_sector);
    ...
    for (uint32_t sector_addr = start_sector; sector_addr <= end_sector; sector_addr += sector_size) {
        ...
        if (fw_is_address_protected(sector_addr)) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Skipping protected sector at 0x%08X" "\033[0m" "\r\n", "FW",sector_addr);
            skipped_count++;
            continue;
        }
        ...
        if (status != FLASH_ADV_OK) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Erase failed at 0x%08X, status=%d" "\033[0m" "\r\n", "FW",sector_addr, status);
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Erased sector at 0x%08X" "\033[0m" "\r\n", "FW",sector_addr);
    }
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Erase complete: %d erased, %d skipped" "\033[0m" "\r\n", "FW",erased_count, skipped_count);
    return FW_RESULT_OK;
}
...
static FlashDownloadResult_t fw_write_buffer_to_flash(uint32_t flash_addr, const uint8_t* buffer, uint32_t size)
{
    ...
    if (!fw_is_address_valid(flash_addr)) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Write address invalid: 0x%08X" "\033[0m" "\r\n", "FW",flash_addr);
        return FW_RESULT_ADDR_INVALID;
    }
    
    
    if (fw_is_address_protected(flash_addr)) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Write address protected: 0x%08X" "\033[0m" "\r\n", "FW",flash_addr);
        return FW_RESULT_ADDR_INVALID;
    }
    ...
    memcpy(g_fw_ram_buffer, buffer, size);
    for (uint32_t i = size; i < word_count * 4; i++) {
        g_fw_ram_buffer[i] = 0xFF;  
    }

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Writing to Flash: addr=0x%08X, size=%d, words=%d" "\033[0m" "\r\n", "FW",flash_addr, size, word_count);
    ...
    FlashAdvStatus_t status = FlashAdv_BulkWriteSimple(
        ...
        flash_addr,
        (uint32_t*)g_fw_ram_buffer,
        word_count
    );
    
    if (status != FLASH_ADV_OK) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Write failed at 0x%08X, status=%d" "\033[0m" "\r\n", "FW",flash_addr, status);
...
void FlashDownload_Init(const FlashDownloadConfig_t* config)
{
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "=== Flash Download Init Start ===" "\033[0m" "\r\n", "FW");
    ...
    g_fw_ctx.flash_handle = FlashAdv_Create(&flash_cfg, 0);
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "User flash range: 0x%08X - 0x%08X" "\033[0m" "\r\n", "FW",g_fw_ctx . config . user_start_addr, g_fw_ctx . config . user_end_addr);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Max firmware size: %d KB" "\033[0m" "\r\n", "FW",g_fw_ctx . config . max_firmware_size / 1024);
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "=== Flash Download Init Done ===" "\033[0m" "\r\n", "FW");
...
FlashDownloadResult_t FlashDownload_OnRequestDownload(uint32_t address, uint32_t size)
{
    ...
    uint32_t end_sector;
    FlashDownloadResult_t erase_result;
    ...
    mapped_addr = (((address) >= 0x08004000 && (address) < 0x08010000) ? ((address) - 0x08004000 + 0x0001A000) : (address));
    ...
    if (g_fw_ctx.state != FW_UPDATE_IDLE) {
        ...
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Wrong state for download: %d" "\033[0m" "\r\n", "FW",g_fw_ctx . state);
        return FW_RESULT_BUSY;
    }
    
    
    if (!fw_is_address_valid(mapped_addr)) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Invalid address: orig=0x%08X, mapped=0x%08X" "\033[0m" "\r\n", "FW",address, mapped_addr);
        return FW_RESULT_ADDR_INVALID;
    }
    
    
    if (size == 0 || size > g_fw_ctx.config.max_firmware_size) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Invalid size: %d (max=%d)" "\033[0m" "\r\n", "FW",size, g_fw_ctx . config . max_firmware_size);
    ...
    g_fw_ctx.total_size = ((size + 255) / 256) * 256;  
    ...
    g_fw_ctx.rx_crc = 0xFFFFFFFF;  
    g_fw_ctx.erased = 0;
    

    
    
    end_addr = mapped_addr + g_fw_ctx.total_size - 1;
    
    
    start_sector = fw_get_sector_start(mapped_addr);
    end_sector = fw_get_sector_start(end_addr);
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Preparing to erase: addr_range=0x%08X-0x%08X, sector_range=0x%08X-0x%08X" "\033[0m" "\r\n", "FW",mapped_addr, end_addr, start_sector, end_sector);

    
    
    erase_result = fw_erase_range(start_sector, end_sector + g_fw_ctx.config.flash_sector_size - 1);
    if (erase_result != FW_RESULT_OK) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Erase failed during RequestDownload" "\033[0m" "\r\n", "FW");
        return erase_result;
    }
...
{
    uint32_t write_addr;
    FlashDownloadResult_t write_result;
    
    if (!g_fw_initialized) {
        return FW_RESULT_NOT_READY;
    }
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " ">>> OnTransferData: seq=%d, len=%d" "\033[0m" "\r\n", "FW",block_sequence_number, len);
    
    
    if (g_fw_ctx.state != FW_UPDATE_READY && g_fw_ctx.state != FW_UPDATE_TRANSFERRING) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Wrong state for transfer: %d" "\033[0m" "\r\n", "FW",g_fw_ctx . state);
        return FW_RESULT_NOT_READY;
    }
    
    
    if (block_sequence_number != g_fw_ctx.expected_sequence) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Sequence mismatch: expected=%d, got=%d" "\033[0m" "\r\n", "FW",g_fw_ctx . expected_sequence, block_sequence_number);
    ...
    if (g_fw_ctx.received_size + len > g_fw_ctx.total_size) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Would exceed total size: received=%d, add=%d, total=%d" "\033[0m" "\r\n", "FW",g_fw_ctx . received_size, len, g_fw_ctx . total_size);
    ...
    write_addr = g_fw_ctx.target_address + g_fw_ctx.received_size - len;
    
    write_result = fw_write_buffer_to_flash(write_addr, data, len);
    if (write_result != FW_RESULT_OK) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Write failed at addr=0x%08X" "\033[0m" "\r\n", "FW",write_addr);
        fw_set_state(FW_UPDATE_ERROR);
        fw_set_error(write_result);
        return write_result;
    }
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Write success at addr=0x%08X, len=%d" "\033[0m" "\r\n", "FW",write_addr, len);
...
FlashDownloadResult_t FlashDownload_OnTransferExit(void)
{
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " ">>> OnTransferExit" "\033[0m" "\r\n", "FW");
    
    
    if (g_fw_ctx.state != FW_UPDATE_TRANSFERRING &&
        g_fw_ctx.state != FW_UPDATE_READY &&
        g_fw_ctx.state != FW_UPDATE_COMPLETE) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Wrong state for transfer exit: %d" "\033[0m" "\r\n", "FW",g_fw_ctx . state);
        return FW_RESULT_NOT_READY;
    }
    
    if (g_fw_ctx.state == FW_UPDATE_COMPLETE) {
        return FW_RESULT_OK;
    }
    
    
    if (g_fw_ctx.received_size < g_fw_ctx.total_size) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Incomplete transfer: received=%d, total=%d" "\033[0m" "\r\n", "FW",g_fw_ctx . received_size, g_fw_ctx . total_size);
...
#line 536 "..\\..\\UDS\\flash_download.c"
    ...
    g_fw_ctx.pending_response = 0;
    
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "=== Firmware Update Complete ===" "\033[0m" "\r\n", "FW");
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Address: 0x%08X, Size: %d bytes (%.2f KB)" "\033[0m" "\r\n", "FW",g_fw_ctx . target_address, g_fw_ctx . total_size, g_fw_ctx . total_size / 1024.0);


    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Received: %d bytes" "\033[0m" "\r\n", "FW",g_fw_ctx . received_size);
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Sectors erased: %d, Blocks written: %d" "\033[0m" "\r\n", "FW",g_fw_ctx . sector_erased_count, g_fw_ctx . block_written_count);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "CRC: 0x%08X" "\033[0m" "\r\n", "FW",g_fw_ctx . rx_crc);
    
    
    if (g_fw_ctx.config.auto_reset_on_complete) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Auto-reset in 1 second..." "\033[0m" "\r\n", "FW");
...
void FlashDownload_Cancel(void)
{
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Cancelling firmware update" "\033[0m" "\r\n", "FW");
...
void FlashDownload_Reset(void)
{
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Resetting firmware update module" "\033[0m" "\r\n", "FW");
...
FlashDownloadResult_t FlashDownload_Erase(uint32_t address, uint32_t size)
{
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Erase requested via 0x31: orig_addr=0x%08X, mapped_addr=0x%08X, size=%d bytes" "\033[0m" "\r\n", "FW",address, mapped_addr, size);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Actual erase already performed at 0x34 (RequestDownload)" "\033[0m" "\r\n", "FW");
...
FlashDownloadResult_t FlashDownload_CalculateCRC(uint32_t address, uint32_t size, uint32_t* crc_result)
{
    ...
    if (!g_fw_initialized || crc_result == 0) {
        return FW_RESULT_NOT_READY;
    }
    ...
    for (uint32_t offset = 0; offset < size; offset += 4) {
        uint32_t word = FlashAdv_ReadWord(g_fw_ctx.flash_handle, mapped_addr + offset);
...
uint16_t FlashDownload_GetFirmwareVersion(void)
{
    ...
    uint32_t version_addr = 0x0001A000 + 0x10;
...
uint32_t FlashDownload_GetFirmwareCRC(void)
{
    ...
    uint32_t crc_addr = 0x0001A000 + 0x0000C000 - 4;
