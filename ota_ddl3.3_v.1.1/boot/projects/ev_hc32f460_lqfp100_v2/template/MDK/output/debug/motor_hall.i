...
#line 3 "..\\..\\Adp\\Motor_hall.c"
#line 4 "..\\..\\Adp\\Motor_hall.c"
#line 5 "..\\..\\Adp\\Motor_hall.c"
#line 6 "..\\..\\Adp\\Motor_hall.c"
...
#line 7 "..\\..\\Adp\\Motor_hall.c"
#line 8 "..\\..\\Adp\\Motor_hall.c"
#line 9 "..\\..\\Adp\\Motor_hall.c"
...
static motor_hall_instance_t g_instances[(4)] = {0};

static motor_hall_instance_t* g_hall_a_map[16] = {0};
static motor_hall_instance_t* g_hall_b_map[16] = {0};

static motor_hall_instance_t* g_current_hall_a_instance = 0;
static motor_hall_instance_t* g_current_hall_b_instance = 0;
...
static void hall_a_irq_callback(void)
{
    ...
    if (interval_us >= (50u) && interval_us <= (200000u)) 
    {
...
static void hall_b_irq_callback(void)
{
    ...
    if (hall_b_last[inst->id] == 1 && hall_b_current == 0) {
        ...
        if (inst->current_direction != detected_direction) {
            ...
            if (inst->direction_confirm_count >= (3)) {
...
static void register_hall_irq(motor_hall_instance_t* inst, uint8_t is_hall_a)
{
    ...
    stcExtiConfig.u32Filter = (0UL);
    stcExtiConfig.u32FilterClock = (0UL);
    stcExtiConfig.u32Edge = ((0x00000002UL));
    EXTINT_Init(eirq_ch, &stcExtiConfig);
    
    GPIO_StructInit(&stcPortInit);
    stcPortInit.u16PinDir = (0U);
    stcPortInit.u16PinAttr = (0U);
    stcPortInit.u16PullUp = ((0x0040U));
    LL_PERIPH_WE((1UL << 2U));
    ...
    __NVIC_SetPriority(irqn, inst->config.irq_priority);
...
static void update_rpm_filter(motor_hall_instance_t* inst, float new_rpm)
{
    ...
    inst->write_index = (inst->write_index + 1) % (6);
    
    if (inst->valid_count < (6)) {
        inst->valid_count++;
    }
...
static float calculate_average_interval(motor_hall_instance_t* inst)
{
    if (inst->last_pulse_interval >= (50u) && 
        inst->last_pulse_interval <= (200000u)) {
...
static float interval_to_rpm(motor_hall_instance_t* inst, uint32_t interval_us)
{
    if (interval_us == 0 || interval_us > (200000u)) {
        return 0;
    }
...
static void perform_stall_detection(motor_hall_instance_t* inst)
{
    ...
    if (inst->current_rpm > 0 && time_since_last_pulse > ((50u) * 1000ul)) {
    ...
    if (time_since_last_pulse > ((500u) * 1000ul)) {
        inst->stalled = 1;
        inst->current_rpm = 0;
        inst->filtered_rpm = 0;
        inst->is_running = 0;
...
static void check_hall_status(motor_hall_instance_t* inst)
{
    ...
    if (current_time - last_check_time[inst->id] >= (1000)) {
...
motor_hall_handle_t motor_hall_create(const motor_hall_config_t* config)
{
    ...
    if (i >= (4)) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "motor_hall_create: No available instance!\r\n" "\033[0m" "\r\n", "MAIN");
        return 0;
    }
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "motor_hall_create: instance %d\r\n" "\033[0m" "\r\n", "MAIN",i);
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  hall_a_port=%d, hall_a_pin=0x%04X\r\n" "\033[0m" "\r\n", "MAIN",inst->config . hall_a_port, inst->config . hall_a_pin);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  hall_b_port=%d, hall_b_pin=0x%04X\r\n" "\033[0m" "\r\n", "MAIN",inst->config . hall_b_port, inst->config . hall_b_pin);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  eirq_ch_a=%lu, eirq_ch_b=%lu\r\n" "\033[0m" "\r\n", "MAIN",inst->config . eirq_ch_a, inst->config . eirq_ch_b);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  irqn_a=%d, irqn_b=%d\r\n" "\033[0m" "\r\n", "MAIN",inst->config . irqn_a, inst->config . irqn_b);

    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "  irq_src_a=%lu, irq_src_b=%lu\r\n" "\033[0m" "\r\n", "MAIN",inst->config . irq_src_a, inst->config . irq_src_b);
    ...
    nbDelay_Init(&inst->hall_check_timer, (1000));
...
uint8_t motor_hall_is_running(motor_hall_handle_t handle)
{
    ...
    return (time_since_last_pulse <= ((50u) * 1000ul));
}
