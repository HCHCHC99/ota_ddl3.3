...
Param_Debug_t    g_ParamDebug;

static uint16_t s_curr_sec = 62;
static uint32_t s_curr_addr = 0;
...
static uint32_t CalcParamCRC(const void *pBuf, uint32_t paramSize, uint32_t checksumOffset)
{
    ...
    stcCrcInit.u32Protocol = ((0x00000002UL));
    stcCrcInit.u32InitValue = 0xFFFFFFFFUL;
    stcCrcInit.u32RefIn = ((0x00000004UL));
    stcCrcInit.u32RefOut = ((0x00000008UL));
    stcCrcInit.u32XorOut = ((0x00000010UL));

    CRC_Init(&stcCrcInit);
    CRC_CRC32_AccumulateData((4U), (const uint32_t*)pBuf, word_len, &crc_result);
...
static void UpdateDebugInfo(int32_t res, uint32_t paramSize)
{
    __disable_irq();
    g_ParamDebug.curr_sec = s_curr_sec;
    g_ParamDebug.curr_addr = s_curr_addr;
    g_ParamDebug.next_addr = s_curr_addr + paramSize;
    g_ParamDebug.last_res = res;
    __enable_irq();
}
...
static int32_t Internal_Erase(uint32_t address)
{
    ...
    return (status == HC32FLASH_OK) ? 0 : -1;
}
...
static int32_t Internal_WriteBuffer(uint32_t addr, const uint32_t* buffer, uint32_t word_count)
{
    ...
    {
        if (Internal_WriteWord(addr + i * 4, buffer[i]) != 0)
        {
            return -1;
        }
    }
...
static _Bool Internal_VerifyBuffer(uint32_t addr, const uint32_t* buffer, uint32_t word_count)
{
    uint32_t i;
    for (i = 0; i < word_count; i++)
    {
...
int32_t Param_Init(const Param_Config_t *pConfig, void (*pSetDefaults)(void))
{
    ...
    if ((pConfig == 0) || (pConfig->pParamBuf == 0) || (pSetDefaults == 0))
    {
        return -3;
    }
    ...
    for (s = 62; s >= 56; s--)
    {
        uint32_t addr = s * 0x2000;
        uint32_t sector_end = (s + 1) * 0x2000;
    ...
    else
    {
        ((void)0);
        pSetDefaults();
        Internal_Erase(62 * 0x2000);
        s_curr_sec = 62;
        s_curr_addr = 62 * 0x2000;
    ...
    UpdateDebugInfo(0, pConfig->paramSize);
    return 0;
}
...
int32_t Param_Save(const Param_Config_t *pConfig)
{
    ...
    if ((pConfig == 0) || (pConfig->pParamBuf == 0))
    {
        return -3;
    }
    ...
    while (retry < 7)
    {
        write_addr = s_curr_addr;

         
        if (write_addr + pConfig->paramSize > (s_curr_sec + 1) * 0x2000)
        {
            uint32_t eraseCnt = GetField32(pConfig->pParamBuf, pConfig->eraseCntOffset);

            if (eraseCnt < 10000)
            {
                if (Internal_Erase(s_curr_sec * 0x2000) == 0)
                {
                    write_addr = s_curr_sec * 0x2000;
            ...
            {
                s_curr_sec = (s_curr_sec <= 56) ? 62 : (s_curr_sec - 1);
                write_addr = s_curr_sec * 0x2000;
                Internal_Erase(s_curr_sec * 0x2000);
                SetField32(pConfig->pParamBuf, pConfig->eraseCntOffset, 1);
            }
        }
        ...
        if (Internal_WriteBuffer(write_addr, (const uint32_t*)pConfig->pParamBuf, paramWords) == 0)
        {
            if (Internal_VerifyBuffer(write_addr, (const uint32_t*)pConfig->pParamBuf, paramWords))
            {
                 
                s_curr_addr = write_addr + pConfig->paramSize;
                g_ParamDebug.save_count++;
                UpdateDebugInfo(0, pConfig->paramSize);
    ...
    UpdateDebugInfo(-1, pConfig->paramSize);
    ((void)0);
    return -1;
}
...
void Param_Debug_EraseAll(const Param_Config_t *pConfig, void (*pSetDefaults)(void))
{
    ...
    for (s = 62; s >= 56; s--)
    {
        Internal_Erase(s * 0x2000);
    }

    Param_Init(pConfig, pSetDefaults);
    ((void)0);
}
