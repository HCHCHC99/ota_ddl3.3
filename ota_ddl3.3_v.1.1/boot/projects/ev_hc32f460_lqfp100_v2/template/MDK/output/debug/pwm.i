...
static _Bool pwmCalcPeriodValue(uint32_t freqHz, uint8_t countMode, uint32_t *period);
...
static pwm_error_t pwmStartRamp(pwm_t *pwm, uint16_t targetDuty, 
                                 uint16_t rampTimeMs, _Bool useCurrent);
...
static uint32_t pwmCalcCompareValue(uint32_t period, uint16_t dutyPercent)
{
    ...
    temp = (uint64_t)period * dutyPercent * (1000U) / 100U;
    return (uint32_t)((temp + ((1000U) / 2U)) / (1000U));
}
                                 ...
                                 uint16_t rampTimeMs, _Bool useCurrent)
{
    ...
    if (useCurrent) {
        ...
        startDuty = (targetDuty > pwm->dutyPercent) ? 0U : (100U);
    }
    ...
    pwm->ramp.totalSteps = dutyDiff / (1U);
    ...
    if (pwm->ramp.stepDelayMs < (10U)) {
        pwm->ramp.stepDelayMs = (10U);
               ...
               pwm_active_t active)
{
    ...
    if ((0 == tmr) || (dutyPercent > (100U)) || (freqHz < (1U))) {
        return pwm;
    }
    ...
    if (active == PWM_ACTIVE_HIGH) {   
        pwmInit.u16StartPolarity = (0x0U);
        pwmInit.u16StopPolarity = (0x0U);
        pwmInit.u16CompareMatchPolarity = (0x1U);
        pwmInit.u16PeriodMatchPolarity = (0x0U);
    } else {                           
        pwmInit.u16StartPolarity = (0x1U);
        pwmInit.u16StopPolarity = (0x1U);
        pwmInit.u16CompareMatchPolarity = (0x0U);
        pwmInit.u16PeriodMatchPolarity = (0x1U);
    }
...
pwm_error_t PWM_SetFrequency(pwm_t *pwm, uint32_t freqHz)
{
    ...
    for (ch = (0U); ch <= (7U); ch++) {
        ...
        if (tempCompare > 0U) {
            uint64_t temp = (uint64_t)tempCompare * (100U) * (1000U);
            dutyPercent[ch] = (uint16_t)((temp + (pwm->periodValue * (1000U) / 2U)) 
                                         / (pwm->periodValue * (1000U)));
            channelMask |= (1UL << ch);
        }
    }
    ...
    TMRA_SetPeriodValue(pwm->tmr, newPeriod);  
...
pwm_error_t PWM_StartRampFromCurrent(pwm_t *pwm, uint16_t targetDuty, uint16_t rampTimeMs)
{
    return pwmStartRamp(pwm, targetDuty, rampTimeMs, 1);
}
...
pwm_error_t PWM_StartRamp(pwm_t *pwm, uint16_t targetDuty, uint16_t rampTimeMs)
{
    return pwmStartRamp(pwm, targetDuty, rampTimeMs, 0);
}
...
pwm_error_t PWM_Update(pwm_t *pwm)
{
    ...
    if (pwm->state != PWM_STATE_RAMPING) {  
        return PWM_OK;
    }
    ...
    if (elapsed >= pwm->ramp.stepDelayMs) {  
        pwm->ramp.currentStep++;
        
        if (pwm->ramp.currentStep >= pwm->ramp.totalSteps) {
            pwm->ramp.currentDuty = pwm->ramp.targetDuty;  
        ...
        pwmWriteDuty(pwm, pwm->ramp.currentDuty);  
